...
import { FALLBACK_CN_MODELS, FALLBACK_GLOBAL_MODELS } from './catalog.ts'
import { WORKBUDDY_DUAL_VERSION } from './version.ts'
import { isHeartbeatProcessAlive, readHostHeartbeat } from './host-heartbeat.ts'
...
function makeStore(): WorkBuddyDualCredentialStore {
...
async function status(jsonOutput: boolean): Promise<number> {
  ...
  let cnCredits: number | undefined
  let globalCredits: number | undefined

  if (cnStatus.state === 'signed-in') {
    try {
  ...
  if (globalStatus.state === 'signed-in') {
    try {
  ...
  if (jsonOutput) {
    printJson({
      ...
      cn: {
        ...
        nickname: cnStatus.state === 'signed-in' ? cnStatus.nickname : undefined,
        credits: cnCredits,
      },
      global: {
        status: globalStatus.state,
        nickname: globalStatus.state === 'signed-in' ? globalStatus.nickname : undefined,
